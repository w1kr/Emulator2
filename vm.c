...
static vm_ins_result vm_push (vm_struct *vm) {
    ...
    sp = vm->regs[VM_REG_SP];
    if(sp < sizeof(vmopvalue_t))
        return VM_RESULT_INVALID_OPERAND;

    sp -= sizeof(vmopvalue_t);
    memcpy(&vm->memory[sp], &value, sizeof(vmopvalue_t));
    vm->regs[VM_REG_SP] = sp;
...
static vm_ins_result vm_pop (vm_struct *vm) {
    ...
    ins = vm_get_current_instruction(vm);
    sp = vm->regs[VM_REG_SP];
    if(sp > VM_MEM_SIZE - sizeof(vmopvalue_t))
        return VM_RESULT_INVALID_OPERAND;

    memcpy(&value, &vm->memory[sp], sizeof(vmopvalue_t));

    if(vm_set_operand(vm, &ins->op1, value) != VM_RESULT_OK)
        return VM_RESULT_INVALID_OPERAND;

    vm->regs[VM_REG_SP] = sp + sizeof(vmopvalue_t);
